...
@Testcontainers
@SpringBootTest(properties = ["spring.datasource.url=jdbc:tc:postgresql:13.2:///?TC_DAEMON=true&TC_INITSCRIPT=file:src/test/resources/data.sql"])
class EmailServiceTest @Autowired constructor(
    ...
    companion object {
        ...
        private val CHECK_WEB_DURATION = Duration.ofSeconds(25)
    ...
    fun cleanup() {
        if (CHECK_WEB_UI) {
            ...
            log.warn { "\tmailhog ui: $MAILHOG_UI_URL" }
            log.warn { "Go sleep" }
            Thread.sleep(CHECK_WEB_DURATION)
        }
    }
